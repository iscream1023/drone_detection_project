...
sd = torch.load(r"C:\Users\haggi\PycharmProjects\drone_detect_project\last_acc0.9993.pth",map_location=device)
...
video_path = root+r"\videos\more_object.mp4"
...
while True:
    ...
    if len(tids):
        ...
        for i, (tid, box) in enumerate(zip(tids, boxes)):
            x1, y1, x2, y2 = box
            p = float(prob[i].item())
            c = int(pred[i].item())
            ...
            if cls_name == 'drone':
                ...
                color = (0,0,255)
            print(f"tid={tid} → {cls_name}  score={p:.3f}")
            cv2.putText(frame, f"tid={tid} → {cls_name}  score={1-p:.3f}",
...
print(f"total tid length = {total_cand}")
